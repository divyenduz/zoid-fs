declare module "node-fuse-bindings" {
  ...
  interface MountOptions {
    ...
    fgetattr?(
      path: string,
      fd: number,
      cb: (code: number, stats: Stats) => void
    ): void;
    ...
    readlink?(
      path: string,
      cb: (code: number | null, targetPathname: string) => void
    ): void;
    ...
    listxattr?(
      ...
      buffer: Buffer,
      length: number,
      cb: (code: number, reqBufSize: number) => void
    ): void;
    ...
    write?(
      ...
      length: number,
      position: number,
      cb: (bytesWrittenOrErr: number) => void
    ): void;
